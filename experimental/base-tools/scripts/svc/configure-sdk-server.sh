...
if [ -n "${SEALOS_DEVBOX_JWT_SECRET:-}" ] && [ "$DEVBOX_ENV" != "production" ]; then
	# SEALOS_DEVBOX_JWT_SECRET exists and is non-empty AND DEVBOX_ENV is not production
	# start the longrun service.
...
if [ "$EXIT_CODE" = "101" ]; then
	echo "Devbox SDK Server stopped due to missing required environment variable SEALOS_DEVBOX_JWT_SECRET. Preventing s6 restart."
